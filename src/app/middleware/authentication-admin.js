...
const authentication_admin = {
    
  async verifyAccessTokenAdmin(req, res, next) {
    ...
    // Authenticate token
    try {
      const admin = await JWT.verify(token, process.env.ACCESS_TOKEN_SECRET);
      req.admin = admin;
      next();
    } catch (error) {
  ...
  async verifyRefreshTokenAdmin(req, res, next) {
...
module.exports = authentication_admin;
